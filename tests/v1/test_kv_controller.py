...
from lmcache.v1.cache_controller.message import (
    KVAdmitMsg, KVEvictMsg, LookupMsg, FullLookupMsg
)
...
async def test_kv_chunk_lifecycle(instance_id, worker_id, location):
    ...
    for start, end, key in kv_controller.token_database.process_tokens(tokens, make_key=False):
        assert isinstance(key, int)
        
        # Test admit
        admit_msg = KVAdmitMsg(instance_id=instance_id, worker_id=worker_id, key=key, location=location)
        ...
        # Test evict
        evict_msg = KVEvictMsg(instance_id=instance_id, worker_id=worker_id, key=key, location=location)
...
async def test_kv_chunk_multiple_instances():
    ...
    for start, end, key in kv_controller.token_database.process_tokens(tokens, make_key=False):
        ...
        evict_msg = KVEvictMsg(instance_id="instance1", worker_id=0, key=key, location="cpu")
        await kv_controller.evict(evict_msg)
...
async def test_lookup_functionality(token_length):
    ...
    for start, end, key in kv_controller.token_database.process_tokens(tokens, make_key=False):
        assert isinstance(key, int)
        admit_msg = KVAdmitMsg(instance_id="test_instance", worker_id=0, key=key, location="cpu")
        await kv_controller.admit(admit_msg)
    
    # Test basic lookup
    lookup_msg = LookupMsg(tokens=tokens.tolist())
...
async def test_full_lookup_functionality(token_length):
    ...
    # Test full lookup
    full_lookup_msg = FullLookupMsg(tokens=tokens.tolist())
...
async def test_lookup_edge_cases():
    ...
    # Test no match
    empty_tokens = []
    empty_lookup_msg = LookupMsg(tokens=empty_tokens)
    ...
    no_match_msg = LookupMsg(tokens=no_match_tokens.tolist())
...
async def test_full_lookup_edge_cases():
    ...
    # Test no match
    empty_tokens = []
    empty_full_lookup_msg = FullLookupMsg(tokens=empty_tokens)
    empty_full_result = await kv_controller.full_lookup(empty_full_lookup_msg)
    assert empty_full_result.matched_info == []
    
    # Test no match with tokens but no KV chunks
    no_match_tokens = generate_tokens(500, "cpu")
    no_match_full_msg = FullLookupMsg(tokens=no_match_tokens.tolist())
...
async def test_edge_cases_and_errors():
    ...
    for start, end, key in kv_controller.token_database.process_tokens(tokens, make_key=False):
        ...
        admit_msg1 = KVAdmitMsg(instance_id="instance1", worker_id=0, key=key, location="cpu")
        admit_msg2 = KVAdmitMsg(instance_id="instance2", worker_id=0, key=key, location="disk")
        ...
        non_existent_msg = KVEvictMsg(instance_id="nonexistent", worker_id=0, key="nonexistent", location="cpu")
...
async def test_concurrent_operations():
    ...
    async def add_kv_chunk(i):
        key = keys[i]
        admit_msg = KVAdmitMsg(instance_id=f"instance_{i}", worker_id=i, key=key, location="cpu")
        await kv_controller.admit(admit_msg)
        return key
    ...
    async def remove_kv_chunk(i):
        key = keys[i]
        evict_msg = KVEvictMsg(instance_id=f"instance_{i}", worker_id=i, key=key, location="cpu")
        await kv_controller.evict(evict_msg)
